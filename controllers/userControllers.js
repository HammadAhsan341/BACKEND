...
const checkAvailability = async (req, res) => {
    try {
      const { email, cnic, phonenumber } = req.body;
      ...
      if (cnicExists) {
        return res.status(400).json({ success: false, message: "This CNIC already exists!" });
      }
      ...
      res.status(200).json({ success: true, message: "User details are available." });
    } catch (error) {
